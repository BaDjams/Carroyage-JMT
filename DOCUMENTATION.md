...
- Export de zone (`zdDrawIsobaths`) : la vue part de l'origine de l'image, un pixel monde entier (`zdPlanZone`), celle-là même des tuiles ; avec une déviation, du coin de l'emprise téléchargée, dans le repère tourné du fond.
...
- **Netteté** : les tuiles sont collées à leur taille native (256 px) et à des positions entières, sans aucun rééchantillonnage — comme MOBAC. Sans déviation, les pixels livrés sont ceux des tuiles. En carroyage rapide, le pivot est décalé de moins d'un demi-pixel pour que la carte tombe sur un pixel entier ; en export de zone, l'image part du coin nord-ouest arrondi à un pixel monde entier (`zdPlanZone`). Grille, KML et géoréférencement partent de ce même pixel
- **Upscale** : un seul agrandissement, d'un facteur ENTIER — le premier qui atteint la cible (`exportUpscaleFactor` en carroyage rapide : 2160 px de haut ; `zdPlanZone` en export de zone : 3840 px sur le grand côté et 2160 px de haut, ×16 au plus). Chaque pixel de tuile devient un carré de k × k pixels identiques, recopié sans lissage : aucune valeur recalculée. Seule une déviation rééchantillonne, une fois, depuis la carte native
- **Taille annoncée** (export de zone, `updateZoneImageSize`) : taille de l'image, format de papier conseillé (plus grand format A imprimé à 300 ppp, puis à 150 ppp), avertissement au-delà de 50 Mpx ; téléchargement bloqué pour un JPEG de plus de 65 535 px de côté ou un GeoTIFF UTM à fabriquer par bandes
- **Par bandes** (export de zone, `zoneBands.js`) : au-delà de 16 384 px de côté ou 64 Mpx, l'image est rendue bloc par bloc et le fichier écrit au fil de l'eau — PNG et JPEG par `imageStream.js` (copie conforme de CadoTour), GeoTIFF et GeoTIFF JPEG par `createGeoTiffStream` (bandes de 32 ou 64 lignes, BigTIFF au-delà de 4 Go). Les surcouches (isobathes, grilles, KML, points, cartouche) sont dessinées une fois dans un contexte enregistreur, puis rejouées sur chaque bloc ; l'encre adaptative lit une vignette du fond de toute l'image. Le fond de chaque bloc est identique au pixel près à l'image d'un seul tenant ; seul le lissage d'un trait fin peut varier de quelques niveaux, sur une fraction infime des pixels
...
- *Déviation* : dans les deux modes, le fond pivote autour du point de référence du carroyage (en export de zone, `zdPlanZone(..., rotationPivot)`), comme dans les KML. Un code tourné donne donc la même grille sur le terrain d'un mode à l'autre.

...
const WGS84_to_UTM = (function() {
    ...
    function fromLatLon(lat, lon, forceZone = null) {
        ...
        const lonRad = toRadians(lonTemp);
        ...
        const UTMEasting = (k0 * N * (A + (1 - T + C) * A ** 3 / 6 + (5 - 18 * T + T ** 2 + 72 * C - 58 * eccPrimeSquared) * A ** 5 / 120) + 500000.0);
        let UTMNorthing = (k0 * (M + N * Math.tan(latRad) * (A ** 2 / 2 + (5 - T + 9 * C + 4 * C ** 2) * A ** 4 / 24 + (61 - 58 * T + T ** 2 + 600 * C - 330 * eccPrimeSquared) * A ** 6 / 720)));
...
async function generateUTMGrid() {
    ...
    try {
        ...
        if (isNaN(nwLat) || isNaN(nwLon) || isNaN(seLat) || isNaN(seLon)) throw new Error("Format de coordonnées invalide. Utilisez 'Lat, Long'.");
...
function createUTM_KML(eastingLines, northingLines, boundaryLines, intermediateLabels, config) {
    ...
    if (intermediateLabels.length > 0) {
        kml += `<Folder><name>Étiquettes (carroyage 5km)</name>${intermediateLabels.map(label => `<Placemark><name>${label.name}</name><styleUrl>#utmIntermediateLabelStyle</styleUrl><Point><coordinates>${label.coordinates.join(',')}</coordinates></Point></Placemark>`).join('')}</Folder>`;
    }
    if (boundaryLines.length > 0) {
        kml += `<Folder><name>Frontières de Zone</name>${boundaryLines.map(line => `<Placemark><name>${line.name}</name><styleUrl>#boundaryLineStyle</styleUrl><LineString><tessellate>1</tessellate><coordinates>${line.coordinates.map(c => c.join(',')).join(' ')}</coordinates></LineString></Placemark>`).join('')}</Folder>`;
    }
    kml += `</Document></kml>`;
    return kml;
}
...
function createKMLPlacemarkForLine(line, lineStyleUrl, labelStyleUrl) {
    ...
    return `
      <Placemark>
        <name>${line.name}</name><styleUrl>${lineStyleUrl}</styleUrl>
        <LineString>
      ...
      <Placemark>
        <name>${line.name}</name><styleUrl>${labelStyleUrl}</styleUrl><Point><coordinates>${startPoint}</coordinates></Point>
      </Placemark>
      <Placemark>
        <name>${line.name}</name><styleUrl>${labelStyleUrl}</styleUrl><Point><coordinates>${endPoint}</coordinates></Point>
      </Placemark>`;
}
